...
When fixing a bug in the manifest writer, test **both insertion orders**. The
one defect found by the first real-world run
([docs/field-report-2026-08-11.md](docs/field-report-2026-08-11.md), F1) was
invisible to the suite precisely because a scoped pin and a top-level pin on the
same package were never staged together in both directions.
...
[docs/field-report-2026-08-11.md](docs/field-report-2026-08-11.md) is a worked
example — read §4 (Method) and §5 (Limits) before doing your own. The method in
brief:

1. **Never trust the tool's own summary.** Diff it against the written
   `package.json` and the resulting lockfile. That discrepancy is how F1 was
   found; an analysis that starts from the summary cannot see it.
